...
class StationBrain(object):
	...
	def readRGB(self, uid):
		...
		pn532.mifare_classic_authenticate_block(uid, RGB_BLOCK, PN532.MIFARE_CMD_AUTH_B,
											[0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF])
	...
	def writeToCard(self, uid):
		...
		for key in sortedParameterKeys:
			...
			if not pn532.mifare_classic_write_block(computedblock, writedata):
				print('Error! Failed to write to block {0}. Canceling write.'.format(computedblock))
				break
			i += 1

	def writeRGB(self, uid, RGB):
		global RGB_BLOCK
		if not pn532.mifare_classic_authenticate_block(uid, RGB_BLOCK, PN532.MIFARE_CMD_AUTH_B,
											CARD_KEY):
			print('Authorization for block 60 failed.')
